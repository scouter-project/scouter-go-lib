...
import (
	...
	"github.com/scouter-project/scouter-go-lib/common/netio/udpclient"
)
...
func GetInstance() *UDPSender {
	once.Do(func() {
		...
		udpSender.udpClient = udpclient.New(configure.NetCollectorIP, configure.NetCollectorUDPPort)
		udpSender.udpClient.SetUDPMaxBytes(configure.UDPMaxBytes)
		udpSender.running = true
		go udpSender.run()
	})
	return udpSender
...
func (udpSender *UDPSender) send(size int) {
	...
	for i := 0; i < size; i++ {
		...
		if bytes+len(buffer) >= configure.UDPMaxBytes {
