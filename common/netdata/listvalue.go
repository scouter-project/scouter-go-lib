...
func (listValue *ListValue) Write(out *DataOutputX) {
	valueSize := len(listValue.Value)
	out.WriteDecimal(int64(valueSize))
	for i := 0; i < valueSize; i++ {
		out.WriteValue(listValue.Value[i])
	}
}

// Read function reads value from datainputx
func (listValue *ListValue) Read(in *DataInputX) Value {
	valueSize := int32(in.ReadDecimal())
	var i int32
	for i = 0; i < valueSize; i++ {
		listValue.Value = append(listValue.Value, in.ReadValue())
	}
	return listValue
}
